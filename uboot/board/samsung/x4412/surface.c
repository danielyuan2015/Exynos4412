...
struct surface_t * surface_alloc(void * pixels, u32_t w, u32_t h, enum pixel_format fmt)
{
	...
	surface = (struct surface_t *)xmalloc(sizeof(struct surface_t));
	if(! surface)
		return NULL;
...
bool_t surface_point(struct surface_t * surface, s32_t x, s32_t y, u32_t c, enum blend_mode mode)
{
	if(!surface || !surface->maps.point)
		return NULL;

	return (surface->maps.point(surface, x, y, c, mode));
}
