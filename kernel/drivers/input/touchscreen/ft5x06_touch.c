...
unsigned char tp_name[32] = "ft5x06-1024x600";
EXPORT_SYMBOL(tp_name);

static unsigned int SCREEN_MAX_WIDTH = 1024;
static unsigned int SCREEN_MAX_HEIGHT = 600;
...
static unsigned char CTPM_FW_800_480[]=
{
#include "ft5x06_firmware_800_480.h"
};

static unsigned char CTPM_FW_1024_600[]=
{
#include "ft5x06_firmware_1024_600.h"
};

static unsigned char CTPM_FW_768_1024[]=
{
#include "ft5x06_firmware_768_1024.h"
};
...
static uint8_t fts_ctpm_get_i_file_ver(void)
{
	...
	if(strcasecmp(tp_name, "ft5x06-1024x600") == 0)
	{
		ui_sz = sizeof(CTPM_FW_1024_600);
    	if (ui_sz > 2)
        	return CTPM_FW_1024_600[ui_sz - 2];
		else
			return 0xff;
	}
	else if(strcasecmp(tp_name, "ft5x06-768x1024") == 0)
	{
		ui_sz = sizeof(CTPM_FW_768_1024);
    	if (ui_sz > 2)
        	return CTPM_FW_768_1024[ui_sz - 2];
		else
			return 0xff;
	}
	else
	{
		ui_sz = sizeof(CTPM_FW_800_480);
		if (ui_sz > 2)
			return CTPM_FW_800_480[ui_sz - 2];
		else
			return 0xff;
	}
	return 0xff;
...
static int fts_ctpm_fw_upgrade_with_i_file(struct i2c_client * client)
{
	...
	if(strcasecmp(tp_name, "ft5x06-1024x600") == 0)
	{
		pbt_buf = CTPM_FW_1024_600;
		i_ret = fts_ctpm_fw_upgrade(client, pbt_buf, sizeof(CTPM_FW_1024_600));
	}
	else if(strcasecmp(tp_name, "ft5x06-768x1024") == 0)
	{
		pbt_buf = CTPM_FW_768_1024;
		i_ret = fts_ctpm_fw_upgrade(client, pbt_buf, sizeof(CTPM_FW_768_1024));
	}
	else
	{
		pbt_buf = CTPM_FW_800_480;
		i_ret = fts_ctpm_fw_upgrade(client, pbt_buf, sizeof(CTPM_FW_800_480));
	}
...
static void ft5x06_ts_work_func(struct work_struct * work)
{
	...
	if(ft5x06_read_coordinates(ts->client, buf, 33) <= 0)
	{
...
static int ft5x06_iic_probe(struct i2c_client * client, const struct i2c_device_id * id)
{
	...
	client->irq = IRQ_EINT(7);

	/* config interrupt pin */
	s5p_register_gpio_interrupt(EXYNOS4_GPX0(7));
	s3c_gpio_cfgpin(EXYNOS4_GPX0(7), S3C_GPIO_SFN(0xf));
	s3c_gpio_setpull(EXYNOS4_GPX0(7), S3C_GPIO_PULL_NONE);
...
static int __init ft5x06_ts_init(void)
{
	...
	if(strcasecmp(tp_name, "ft5x06-800x480") == 0)
	{
		SCREEN_MAX_WIDTH = 800;
		SCREEN_MAX_HEIGHT = 480;
	}
	else if(strcasecmp(tp_name, "ft5x06-1024x600") == 0)
	{
