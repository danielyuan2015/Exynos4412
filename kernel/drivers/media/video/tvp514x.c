...
static struct tvp514x_reg tvp514x_reg_list_default[] = {
	...
	{TOK_SKIP, 0x15, 0x00},
	/* NTSC timing */
	{TOK_WRITE, REG_AVID_START_PIXEL_LSB, 0x58},
	{TOK_WRITE, REG_AVID_START_PIXEL_MSB, 0x00},
	{TOK_WRITE, REG_AVID_STOP_PIXEL_LSB, 0x28},
	...
	/* NTSC timing */
	{TOK_WRITE, REG_VSYNC_START_LINE_LSB, 0x01},
	{TOK_WRITE, REG_VSYNC_START_LINE_MSB, 0x00},
	{TOK_WRITE, REG_VSYNC_STOP_LINE_LSB, 0x04},
	{TOK_WRITE, REG_VSYNC_STOP_LINE_MSB, 0x00},
	/* NTSC timing */
	{TOK_WRITE, REG_VBLK_START_LINE_LSB, 0x03},
	{TOK_WRITE, REG_VBLK_START_LINE_MSB, 0x00},
	{TOK_WRITE, REG_VBLK_STOP_LINE_LSB, 0x17},
	{TOK_WRITE, REG_VBLK_STOP_LINE_MSB, 0x00},
	/* Reserved */
	{TOK_SKIP, 0x26, 0x00},
	/* Reserved */
	{TOK_SKIP, 0x27, 0x00},
...
static ssize_t regs_read(struct device *dev, struct device_attribute *attr, char *buf)
{
...
static ssize_t regs_write(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct v4l2_subdev *sd = i2c_get_clientdata(client);
	unsigned int val[2] = {0, 0};

	sscanf(buf, "0x%x 0x%x", &val[0], &val[1]);
	tvp514x_write_reg(sd, val[0] & 0xff, val[1] & 0xff); 
	return count;
}
...
static ssize_t channel_read(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	val = tvp514x_read_reg(sd, REG_INPUT_SEL);
	switch(val)
	{
	case 0x0:
		ch = 0;
...
static ssize_t channel_write(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct v4l2_subdev *sd = i2c_get_clientdata(client);
	u32 ch = simple_strtoul(buf, NULL, 10);
	u8 val;
	...
	tvp514x_write_reg(sd, REG_INPUT_SEL, val); 
	return count;
}

static DEVICE_ATTR(regs, 0666, regs_read, regs_write);
static DEVICE_ATTR(channel, 0666, channel_read, channel_write);

static struct attribute * tvp514x_sysfs_entries[] = {
	&dev_attr_regs.attr,
	&dev_attr_channel.attr,
	NULL,
};
static struct attribute_group tvp514x_attr_group = {
	.name	= NULL,
	.attrs	= tvp514x_sysfs_entries,
};
...
static void tvp514x_reg_dump(struct v4l2_subdev *sd)
{
	...
	dump_reg(sd, REG_AUTOSWITCH_MASK);
	...
	dump_reg(sd, REG_STATUS1);
	dump_reg(sd, REG_STATUS2);
	dump_reg(sd, REG_VIDEO_STD_STATUS);
	dump_reg(sd, 0x42);
	dump_reg(sd, 0x43);
}
...
static int tvp514x_s_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_AUTOGAIN:
		...
		if (!err)
			decoder->tvp514x_regs[REG_AFE_GAIN_CTRL].val = value ? 0x0f : 0x0c;
		break;
	}
...
static int tvp514x_dummy_init(struct v4l2_subdev *sd, u32 val)
{
	return 0;
}

static int tvp514x_enum_framesizes(struct v4l2_subdev *sd, struct v4l2_frmsizeenum *fsize)
{
...
static const struct v4l2_subdev_video_ops tvp514x_video_ops = {
	...
	.enum_framesizes = tvp514x_enum_framesizes,
};
...
tvp514x_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	*decoder = tvp514x_dev;
	...
	return sysfs_create_group(&(client->dev.kobj), &tvp514x_attr_group);
}
...
static int tvp514x_remove(struct i2c_client *client)
{
	...
	sysfs_remove_group(&(client->dev.kobj), &tvp514x_attr_group);
